...
use RefactorF4Acc::Parser::Expressions
  qw( get_vars_from_expression parse_expression );
...
"INFO: The include file $inc contains both parameters and commons, attempting to split out params_$inc.\n"
			...
			$stref =
			  __find_parameter_used_in_inc_and_add_to_Only( $inc, $stref );
			$has_pars = 0;
		}
		elsif ($has_commons) {
...
# I do remove IMPLICIT NONE because I will add it later to the module
	...
	my $insert_cond_subref = sub {
		...
		return ( exists $info->{'Include'}
			  and $info->{'Include'}{'Name'} eq $n_inc )
		  ? 1
		  : 0;
	};
	my $merged_annlines =
	  splice_additional_lines_cond( $stref, $inc, $insert_cond_subref,
		$old_annlines, \@n_inc_annlines, 0, 1, 1 );
...
sub precondition_all {
	( my $f, my $stref, my $is_source_file_path ) = @_
...
#                  say "PARAM SET: $var => ".Dumper( $Sf->{'Parameters'}{'Set'}{$var} );
						...
						my $param_decl = {
							'Indent' => $info->{'Indent'},
							'Type' =>
							  $Sf->{'LocalParameters'}{'Set'}{$var}{'Type'},
							'Attr' =>
							  $Sf->{'LocalParameters'}{'Set'}{$var}{'Attr'},
							'Dim'       => [],
							'Parameter' => 'parameter',
							'Name'      => [ $var, $val ],
						...
						}; # $Sf->{'LocalParameters'}{'Set'}{$var};# {'Name' => $var};#
