...
sub pass_rename_array_accesses_to_scalars {
    ...
    # All Fixes are off by default, list them in $Config{'FIXES'} to enable them
            ...
            [ \&_update_call_args, \&_add_assignments_for_called_subs ],
            [
                # What does this do?
                \&_make_dim_vars_scalar_consts_in_sigs
            ],
...
sub _rename_array_accesses_to_scalars {
    ( my $stref, my $f ) = @_;
...
# 	* DO index range expressions (I'm lazy, FIXME!)
        my $pass_rename_array_accesses_in_exprs = sub {
            ...
            if ( exists $info->{'Signature'} ) {
                ...
                if ( scalar @{ $info->{'Rhs'}{'Vars'}{'List'} } == 1
                    and $info->{'Rhs'}{'Vars'}{'List'}[0] =~ /_ptr/ )
                {
                    warn
'FIXME: What we want is that only array variables with IndexVars are renamed. So constant indices should stay as they are';
...
# I have $state->{'Args'} for that. But the RHS is an expression which could have many vars
                    ( my $ast, $state ) =
                      _scalarise_array_accesses_in_ast( $stref, $f, $state,
                        $info->{'Rhs'}{'ExpressionAST'}, 'In' );
 ...
 # Rename all array accesses in the LHS AST. This updates $state->{'StreamVars'}
                ...
                $state->{'IndexVars'} = {
                    %{ $state->{'IndexVars'} },
                    %{ $info->{'Lhs'}{'IndexVars'}{'Set'} }
                };
                for my $var ( @{ $info->{'Rhs'}{'Vars'}{'List'} } ) {
                    if ( $info->{'Rhs'}{'Vars'}{'Set'}{$var}{'Type'} eq 'Array'
                        and
                        exists $info->{'Rhs'}{'Vars'}{'Set'}{$var}{'IndexVars'}
                      )
                    {
                        $state->{'IndexVars'} = {
                            %{ $state->{'IndexVars'} },
                            %{
                                $info->{'Rhs'}{'Vars'}{'Set'}{$var}{'IndexVars'}
                            }
                        };
                    }
                }
     ...
     # Rename all array accesses in the AST. This updates $state->{'StreamVars'}
                ...
                for my $var ( @{ $info->{'Cond'}{'Vars'}{'List'} } ) {
                    if (
                        ...
                        and
                        exists $info->{'Cond'}{'Vars'}{'Set'}{$var}{'IndexVars'}
                      )
                    {
                        $state->{'IndexVars'} = {
                            %{ $state->{'IndexVars'} },
                            %{
                                $info->{'Cond'}{'Vars'}{'Set'}{$var}
                                  {'IndexVars'}
                            }
                        };
                    }
...
# This is very odd: if there are Read accesses, we get them, else we use the Write accesses?
            my $accesses =
              exists $stref->{'Subroutines'}{$f}{'ArrayAccesses'}{0}{'Arrays'}
              {$var}{'Read'}
              ? $stref->{'Subroutines'}{$f}{'ArrayAccesses'}{0}{'Arrays'}{$var}
              {'Read'}{'Accesses'}
              : $stref->{'Subroutines'}{$f}{'ArrayAccesses'}{0}{'Arrays'}{$var}
              {'Write'}{'Accesses'};
            ...
            my $array_dims =
              $stref->{'Subroutines'}{$f}{'ArrayAccesses'}{0}{'Arrays'}{$var}
              {'Dims'};
            my ( $offsets_for_scalarised_vars, $ordered_stencil_var_tuple ) =
              _link_scalarised_vars_to_linear_offsets( $var, $accesses,
                $array_dims );
            $stref->{'TyTraLlvmArgTuples'}{$f}{$var} =
...
# );
    ...
    # warn $stream_var. Dumper $state->{'StreamVars'}{$var}{'Set'}{$stream_var};
                ...
                my $array_assignment_line =
                  ...
                  {'ArrayIndexExpr'} . ' = '
                  . $stream_var;
                my $scalar_assignment_rhs_ast = parse_expression(
                    $state->{'StreamVars'}{$var}{'Set'}{$stream_var}
                      {'ArrayIndexExpr'},
                    {}, $stref, $f );
                my $array_assignment_lhs_ast = $scalar_assignment_rhs_ast;
                my $scalar_assignment_rhs_vars =
                  get_vars_from_expression( $scalar_assignment_rhs_ast, {} );
                my $array_assignment_lhs_vars = $scalar_assignment_rhs_vars;

                my $scalar_assignment_rhs_vars_list =
                  [ sort keys %{$scalar_assignment_rhs_vars} ];
                my $array_assignment_lhs_vars_list =
                  $scalar_assignment_rhs_vars_list;
                push @{ $stref->{'Subroutines'}{$f}{'LiftedScalarAssignments'}
                    ...
                    $scalar_assignment_line,
                    {
                        ...
                        'Rhs' => {
                            ...
                            'Vars'          => {
                                'List' => $scalar_assignment_rhs_vars_list,
                                'Set'  => $scalar_assignment_rhs_vars
                            },
                        }
...
# Append the Out access to the List because it is not part of the stencil but we need it for the arguments. There should only be one per array!
            my $found = 0;
            for my $scalarised_access_var (
                sort keys %{ $state->{'StreamVars'}{$var}{'Set'} } )
            {
                if (
                    not exists
                    $offsets_for_scalarised_vars->{$scalarised_access_var}
                    and
                    $state->{'StreamVars'}{$var}{'Set'}{$scalarised_access_var}
                    {'IODir'} eq 'Out' )
                {
                    if ( $found == 0 ) {
                        push @{ $state->{'StreamVars'}{$var}{'List'} },
                          $scalarised_access_var;
                        $found = 1;
                    }
                    else {
                        croak
"ERROR: There should be only one Out access for each array in a Map function: "
                          . Dumper( $state->{'StreamVars'}{$var}{'Set'} );
                    }
                }
            }
            ...
            if ( $found == 0 ) {
                say
"INFO: There should be just one Out access for each array in a Map function: "
                  . Dumper( $state->{'StreamVars'}{$var}{'Set'} )
                  if $I;
            }
...
# What we do is replace the array args with the "tuple" of scalar args from StreamVars
                ...
                for my $arg ( @{ $info->{'Signature'}{'Args'}{'List'} } ) {
                    if ( exists $state->{'StreamVars'}{$arg} ) {
                        $new_args = [
                            @{$new_args},
                            @{ $state->{'StreamVars'}{$arg}{'List'} }
                        ];
                        my $idx = 0;

                        # Here we store the stencil index BASE 1 for Fortran
                        map { $orig_arg_names{$_} = [ $arg, ++$idx ] }
                          @{ $state->{'StreamVars'}{$arg}{'List'} };
                    }
                    else {
                        push @{$new_args}, $arg;
                ...
                $info->{'Signature'}{'Args'}{'Set'} =
                  { map { $_ => $orig_arg_names{$_} } @{$new_args} };
                ...
                if ( exists $state->{'StreamVars'}{$var} ) {
                    ...
                    if ( exists $info->{'ParsedVarDecl'} ) {
                        $info->{'ParsedVarDecl'}{'StreamVars'} =
                          $state->{'StreamVars'}{$var}{'Set'}
                          ; # Every streamvar derived from var has the same type
                        ...
                        if (
                            ...
                            {'Intent'} )
                        {
                            delete $info->{'ParsedVarDecl'}{'Attributes'}
                              {'Intent'};
                        }
...
# 4. Here we update DeclaredOrigArgs
            ...
            @{ $stref->{'Subroutines'}{$f}{'DeclaredOrigArgs'}{'List'} } )
        {
            ...
            if ( not defined $orig_arg ) {
                carp
"Undefined arg in position $_arg_idx in DeclaredOrigArgs for $f";
            }
            if ( exists $state->{'StreamVars'}{$orig_arg} ) {
                my $idx = 0;
                for
                  my $new_arg ( @{ $state->{'StreamVars'}{$orig_arg}{'List'} } )
                {
                    my $new_decl = dclone(
                    ...
                    $new_decl->{'Dim'}           = [];
                    $new_decl->{'IODir'} =
                      $state->{'StreamVars'}{$orig_arg}{'Set'}{$new_arg}
                      {'IODir'};
                    $new_decl->{'ArrayIndexExpr'} =
                      $state->{'StreamVars'}{$orig_arg}{'Set'}{$new_arg}
                      {'ArrayIndexExpr'};
                    if (
                        scalar @{ $state->{'StreamVars'}{$orig_arg}{'List'} } >
                        1 )
                    {
                ...
                delete $stref->{'Subroutines'}{$f}{'DeclaredOrigArgs'}{'Set'}
                  {$orig_arg};
            }
            else {
                if (
                    exists $stref->{'Subroutines'}{$f}{'DeclaredOrigArgs'}
                    {'Set'}{$orig_arg} )
                {
...
# Then we can remove the declarations as well, and store these in LiftedIndexVarDecls
                ...
                if ( exists $state->{'IndexVars'}{$decl_var} ) {
                    unshift @{ $state->{'LiftedIndexVarDecls'}{'List'} },
                      dclone($annline);
            ...
            # Finally we remove the $index_vars from the Args in the Signature
                ...
                for my $arg ( @{ $info->{'Signature'}{'Args'}{'List'} } ) {
                    if ( not exists $state->{'IndexVars'}{$arg} ) {
                        ...
                        delete $stref->{'Subroutines'}{$f}{'DeclaredOrigArgs'}
                          {'Set'}{$arg};
                        my @updated_args_list =
                          grep { $_ ne $arg }
        ...
        ( $stref, $state ) =
          stateful_pass_reverse_inplace( $stref, $f,
            $pass_lift_array_index_calculations,
            $state, '_rename_array_accesses_to_scalars_lift() ' . __LINE__ );
...
# And then we can update $stref->{$Subroutines}{$f} and add LiftedIndexCalcLines and LiftedIndexVarDecls so that when we find a call we can splice in these lines
        $stref->{'Subroutines'}{$f}{'LiftedIndexCalcLines'} =
          dclone( $state->{'LiftedIndexCalcLines'} );
...
# 6. Now we emit the updated code for the subroutine signature, the variable declarations, assignment expressions and ifthen expressions
        ...
        my $pass_emit_updated_code = sub {
            ...
            if ( exists $info->{'Signature'} ) {
                ...
                and exists $info->{'ParsedVarDecl'}{'StreamVars'} )
            {
                ...
                for my $tvar ( sort keys %{ $tvar_rec->{'StreamVars'} } ) {
                    ...
                    my $kind =
                      exists $tvar_rec->{'TypeTup'}{'Kind'}
                      ? '(kind=' . $tvar_rec->{'TypeTup'}{'Kind'} . ')'
                      : '';
...
# So we should only have IODir if $intent is defined, i.e. if the IODir key exists
                    ...
                    if ( exists $tvar_rec->{'StreamVars'}{$tvar}{'IODir'} ) {
                        $rdecl->{'IODir'} =
                          $tvar_rec->{'StreamVars'}{$tvar}{'IODir'};
                    }
                    ...
                    $stref->{'Subroutines'}{$f}{'LiftedStreamVarDecls'}{'Set'}
                      {$tvar} = dclone($annline);
        ...
        $stref->{'Subroutines'}{$f}{'LiftedStreamVarDecls'} =
          { 'Set' => {}, 'List' => [] };
        my $global_state_access = [ $stref, $f ];
        ( $stref, $global_state_access ) =
          stateful_pass_inplace( $stref, $f, $pass_emit_updated_code,
            $global_state_access,
            '_rename_array_accesses_to_scalars_PASS3() ' . __LINE__ );
        $stref->{'Subroutines'}{$f}{'RefactoredArgs'} =
          $stref->{'Subroutines'}{$f}{'DeclaredOrigArgs'};
    }    # IF NOT A KERNEL
       # if ($f =~/f1/) {
    ...
    if ( $f eq $Config{'KERNEL'} ) {
        ...
        my $pass_action = sub {
            ...
            if ( exists $info->{'SubroutineCall'}
                and not exists $stref->{'ExternalSubroutines'}
...
#  croak $subname.': '.Dumper( $stref->{'Subroutines'}{$subname}{'DeclaredOrigArgs'}); #
                ...
                $stref->{'Subroutines'}{$f}{'LiftedVarDecls'}{'Set'} = {
                    %{ $stref->{'Subroutines'}{$f}{'LiftedVarDecls'}{'Set'} },
                    (
                        exists $stref->{'Subroutines'}{$subname}
                          {'LiftedIndexVarDecls'} ? %{
                            $stref->{'Subroutines'}{$subname}
                              {'LiftedIndexVarDecls'}{'Set'}
                          } : ()
                    ),
                    (
                        exists $stref->{'Subroutines'}{$subname}
                    ...
                    %{ $stref->{'Subroutines'}{$f}{'LiftedVarDecls'}{'Set'} } )
                {
                    ...
                    if (
                        not exists $stref->{'Subroutines'}{$f}
                        {'DeclaredOrigLocalVars'}{'Set'}{$lifted_var}
                        and not
                        exists $stref->{'Subroutines'}{$f}{'DeclaredOrigArgs'}
                        {'Set'}{$lifted_var} )
                    {
                        my $var_rec = get_var_record_from_set(
                            $stref->{'Subroutines'}{$subname}{'Vars'},
                            $lifted_var );
                        if ( defined $var_rec ) {
                            $stref->{'Subroutines'}{$f}
                              {'DeclaredOrigLocalVars'}{'Set'}{$lifted_var} =
                ...
                ( $rline, $info ) =
                  _emit_subroutine_call_w_streams( $stref, $f, $annline );
                say $rline if $DBG;
                push @{$rlines}, [ $rline, $info ];
        ...
        $stref->{'Subroutines'}{$f}{'LiftedVarDecls'} =
          { 'Set' => {}, 'List' => {} };
        my $state = [ $stref, $f ];
        ( $stref, $state ) =
          stateful_pass_inplace( $stref, $f, $pass_action, $state,
            '_rename_array_accesses_to_scalars_called_subs() ' . __LINE__ );
        @{ $stref->{'Subroutines'}{$f}{'LocalVars'}{'List'} } =
          sort keys %{ $stref->{'Subroutines'}{$f}{'LocalVars'}{'Set'} };
    ...
    if ( $f eq $Config{'KERNEL'} ) {
        ...
        my $pass_update_call_args = sub {
                ...
                { $info->{'SubroutineCall'}{'Name'} } )
            {
                ...
                {
                    my $call_arg =
                      $info->{'SubroutineCall'}{'ArgMap'}{$sig_arg};

                    # This is only correct if the signature arg is a scalar.
                    my $sig_iodir =
                      $stref->{'Subroutines'}{$subname}{'DeclaredOrigArgs'}
                      {'Set'}{$sig_arg}{'IODir'};
                    if ( $sig_iodir ne 'in' )
                    {    # means the sig arg is a pointer anyway
                        if ( $info->{'SubroutineCall'}{'Args'}{'Set'}{$call_arg}
                            {'Expr'} eq $call_arg . '(1)' )
                        {
                            $info->{'SubroutineCall'}{'Args'}{'Set'}{$call_arg}
                              {'Expr'} = $call_arg;
                        }
                    }
                }
                ...
                my $maybe_label =
                  ...
                  : '';
                my @new_call_exprs =
                  map { $info->{'SubroutineCall'}{'Args'}{'Set'}{$_}{'Expr'} }
                  @{ $info->{'SubroutineCall'}{'Args'}{'List'} };
                my $args_str = join( ',', @new_call_exprs );
                my $rline    = "call $subname($args_str)\n";
                my $updated_expr_ast =
                  parse_expression( "$subname($args_str)", $info, $stref, $f );
        ...
        my $state = [ $stref, $f ];
        ( $stref, $state ) =
          stateful_pass_inplace( $stref, $f, $pass_update_call_args, $state,
            'pass_update_call_args() ' . __LINE__ );
...
# ================================================================================================================================================
# After we've renamed all args in the subroutine definitions, we update the calls as well, but ONLY in the kernel
sub _add_assignments_for_called_subs {
    ( my $stref, my $f ) = @_;
    if ( $f eq $Config{'KERNEL'} ) {
                ...
                { $info->{'SubroutineCall'}{'Name'} } )
            {
                    ...
                    {'LiftedScalarAssignments'} )
                {
                      ...
                      )
                    {
                        ...
                        if (
                            exists $stref->{'Subroutines'}{$subname}
                            {'DeclaredOrigArgs'}{'Set'}{$var} )
                        {
                            my $iodir =
                              exists $stref->{'Subroutines'}{$subname}
                              {'DeclaredOrigArgs'}{'Set'}{$var}{'IODir'}
                              ? $stref->{'Subroutines'}{$subname}
                              {'DeclaredOrigArgs'}{'Set'}{$var}{'IODir'}
                              : 'Unknown';
                    ...
                    {'LiftedArrayAssignments'} )
                {
                    ...
                    {
                        my $var =
                          $lifted_annline->[1]{'Rhs'}{'Vars'}{'List'}[0];
                        if (
                            exists $stref->{'Subroutines'}{$subname}
                            {'DeclaredOrigArgs'}{'Set'}{$var} )
                        {
...
# Here we add the variable declarations for variables used in the lifted assignments
        my $pass_add_decls_lifted_vars = sub {
            ( my $annline, my $state ) = @_;
            ( my $line,    my $info )  = @{$annline};
            ( my $stref,   my $f )     = @{$state};
            ...
            if ( exists $info->{'VarDecl'} ) {
                my $var = $info->{'VarDecl'}{'Name'};
                if (
                    exists $stref->{'Subroutines'}{$f}{'LiftedVarDecls'}{'Set'}
                    {$var} )
                {
                    say "VAR $var from LiftedVarDecls already declared in $f"
                      if $DBG;
                    delete $stref->{'Subroutines'}{$f}{'LiftedVarDecls'}{'Set'}
                      {$var};
                }
            }
        ...
        #	my $state=[$stref,$f];
        ( $stref, $state ) =
          stateful_pass_inplace( $stref, $f, $pass_add_decls_lifted_vars,
            $state, 'pass_add_decls_lifted_vars() ' . __LINE__ );

        my @lifted_var_decls =
          map { $stref->{'Subroutines'}{$f}{'LiftedVarDecls'}{'Set'}{$_} }
          sort keys %{ $stref->{'Subroutines'}{$f}{'LiftedVarDecls'}{'Set'} };
        ...
        my $merged_annlines = splice_additional_lines_cond_inplace(
            $stref,
            $f,
            sub { ( my $al ) = @_; exists $al->[1]{'VarDecl'} ? 1 : 0 },
...
sub _scalarise_array_accesses_in_ast {
    ( my $stref, my $f, my $state, my $ast, my $intent ) = @_;
    ...
    if ( ref($ast) eq 'ARRAY' ) {
        for my $idx ( 0 .. scalar @{$ast} - 1 ) {
            ...
            if ( ref($entry) eq 'ARRAY' ) {
                ( my $entry, $state ) =
                  _scalarise_array_accesses_in_ast( $stref, $f, $state, $entry,
                    $intent );
                $ast->[$idx] = $entry;
            }
            else {
                if ( $idx == 0 and ( ( $entry & 0xFF ) == 10 ) ) {    #'@'
                    ...
                    if (
                        exists $stref->{'Subroutines'}{$f}{'StreamVars'}{$mvar}
                      )
                    {
...
# Ideally I should re-run the analysis for the stream vars
                        ...
                        if (
                            exists $stref->{'Subroutines'}{$f}
                            {'DeclaredOrigArgs'}{'Set'}{$mvar} )
                        {
                            if (
                                exists $state->{'StreamVars'}{$mvar}{'Set'}
                                {$var_str} )
                            {
                                if ( $state->{'StreamVars'}{$mvar}{'Set'}
                                    {$var_str}{'IODir'} ne 'InOut'
                                    and $state->{'StreamVars'}{$mvar}{'Set'}
                                    {$var_str}{'IODir'} ne $intent )
                                {
                                    $state->{'StreamVars'}{$mvar}{'Set'}
                                      {$var_str}{'IODir'} = 'InOut';
                                }
                                ...
                                $state->{'StreamVars'}{$mvar}{'Set'}{$var_str}
                                  = {
                                    'IODir'          => $intent,
                                    'ArrayIndexExpr' => $expr_str
                                  };
...
sub _emit_subroutine_call_w_streams {
    ...
    for my $call_arg ( @{$orig_call_args} ) {
            ...
            {$current_sig_arg} )
        {
            ...
            exists $stref->{'Subroutines'}{$name}{'DeclaredOrigArgs'}{'Set'}
            {$current_sig_arg} )
        {
...
# croak;# Dumper $info->{'SubroutineCall'};
    ...
    ( my $rline, my $rinfo ) =
      emit_subroutine_call( $stref, $f, [ $line, $info ] );
...
sub _link_scalarised_vars_to_linear_offsets {
    ( my $var, my $accesses, my $array_dims ) = @_;
    ...
    for my $offset_str ( sort keys %{$accesses} ) {
        ...
        for my $iter_pair ( @{ $accesses->{$offset_str} } ) {
            ...
            my ( $mult, $offset ) =
              @{$iter_mult_offset};    # @{$accesses->{$offset_str}{$iter_str}};
            $ordered_iter_seq[$iter_idx] =
              __iter_rec_to_scal_str( $iter, $mult, $offset );
        }
    ...
    $ordered_stencil_var_tuple =
      [ map { $scalarised_vars_for_offsets->{$_} } @stencil_order ];
