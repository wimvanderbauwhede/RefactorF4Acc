...
for my $WD (1 .. 32) { 
for my $UNROLL ( 2 .. 4) {
    system("./aux/gen_macros_cfg_SConstruct.pl $UNROLL $WD");

    chdir "$wd/src";
    system("run_cpp.pl -o ../src_${UNROLL}_${WD}_postcpp/");

    chdir "$wd";
	# *  create an inlined version in `refactored-src_{UNROLL}/src_{UNROLL}_postcpp`
    system("refactorF4acc.pl -c ./rf4a_${UNROLL}.cfg");
    ...
    system("./aux/gen_SConstruct_refactored.pl $UNROLL $WD");

    chdir "$wd/refactored-src_${UNROLL}_${WD}/src_${UNROLL}_${WD}_postcpp";
	# * create the module in `./autopar_{UNROLL}`
    system("../../aux/run_autoparallel_compiler_GPU.sh $UNROLL $WD");
	# * patch the module file to change `get_global_id()` calls to `global_id` arguments: 
    chdir "$wd/autopar_${UNROLL}_${WD}";
    ...
    chdir "$wd/patched_autopar_${UNROLL}_${WD}";
    ...
    chdir "$wd/patched_autopar_${UNROLL}_${WD}/MemoryReduction";
    system("refactorF4acc.pl -c rf4a_inline_${UNROLL}");

    chdir "$wd/patched_autopar_${UNROLL}_${WD}/mem_reduced_inlined/Generated";
    system("../../../aux/patch_inlined_code.pl $UNROLL $WD");
    # -  final code in `patched_autopar_{UNROLL}_${WD}/mem_reduced_inlined/Generated/Patched`
    system("cp -r $wd/patched_autopar_${UNROLL}_$WD/mem_reduced_inlined/Generated/Patched $wd/unroll_${UNROLL}_${WD}")
