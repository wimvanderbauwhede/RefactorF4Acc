...
contains
      ...
      use params_SIZE, only : lelv, lx1, lx2, ly1, ly2, lz1, lz2
...
!!      use params_INPUT ! ONLY LIST EMPTY
      ...
      unz)
      use params_SIZE, only : ldim, lz1, lelt, ly1, lx1, lelv
!!      use params_INPUT ! ONLY LIST EMPTY
      implicit none
      ...
      use params_SIZE, only : lhis, lelt, lelv, lz2, ly1, lx2, ldimt1, lelg, maxobj, lz1, ly2, lx1, &
       maxmbr
!!      use params_TOTAL ! ONLY LIST EMPTY
      implicit none
      integer, parameter :: maxrts=1000
    ...
    6   format(i8,1p4e19.11,2x,i5,a5)
      enddo
      trx(1:lx1,1:ly1,1:lz1) = reshape(xm0(1,1,1,1:512), shape(trx))
      trz(1:lx1,1:ly1,1:lz1) = reshape(xm0(1,1,1,513:1024), shape(trz))
      return
      ...
      use params_SIZE, only : lelv, lx1, lelt, ly1, lz1
!!      use params_TOTAL ! ONLY LIST EMPTY
      implicit none
      integer, parameter :: maxrts=1000
      logical, intent(In) :: if3d
