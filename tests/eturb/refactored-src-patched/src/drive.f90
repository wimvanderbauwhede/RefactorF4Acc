      program main
      use singleton_module_src_abl
      use params_SIZE, only : maxmbr, lx1, lz1, ly2, ldimt, ldimt1, lx2, maxobj, lelg, lz2, ldim, ly1, lelv, lhis, lelt
...
!!      use params_ZPER ! ONLY LIST EMPTY
      ...
      real, dimension(1:lx1*ly1*lz1*6*lelv) :: sij
      ...
      real, dimension(1:lxyz) :: ur
      real, dimension(1:lxyz) :: us
      real, dimension(1:lxyz) :: ut
      real, dimension(1:ldim) :: utldo
      ...
      call userchk(base_flow,cs,dg2,dgtq,domain_length,dpdx_mean,dpdy_mean,dpdz_mean,dragpx,dragpy,dragpz,dragvx,dragvy,dragvz,dragx,dragy,dragz,ediff,ffx_new,ffy_new,ffz_new,flow_rate,icall,nekcomm,nekgroup,nekreal,nid,np,pm1,scale_vf,sij,snrm,torqpx,torqpy,torqpz,torqvx,torqvy,torqvz,torqx,torqy,torqz,trx,trz,ur,us,ut,utldo,vr,vs,vt,wr,ws,wt,xm0,xsec,ym0,zm0,nelv_GLOB,nx1_GLOB,ny1_GLOB,nz1_GLOB,ifuservp_GLOB,t_GLOB,istep_GLOB,vx_GLOB,ym1_GLOB,iostep_GLOB,pr_GLOB,ndim,jp,xm1,zm1,vy,vz,nelt,param,if3d,dxm1,dxtm1,jacmi,rxm1,sxm1,txm1,rym1,sym1,tym1,rzm1,szm1,tzm1,ifaxis,nrout,rname,dct,ncall,dcount,tmxmf,ifneknek,ttotal,etimes,tprep,ttime,nvtot,ifsync,tgop,ngop,nobj,nhis,hcode,lochis,cbc,nmember,lglel,object,bm1,volvm1,dt,vdiff,ifield,gllnid,gllel,nio,time,ifsplit,ixm21,iytm21,iztm21,vmult,tcol2,ta2s2,ifrzer,dytm1,dym1,dam1,datm1,dcm1,dctm1,eface1,skpdat,area,unx,uny,unz)
      end program main
