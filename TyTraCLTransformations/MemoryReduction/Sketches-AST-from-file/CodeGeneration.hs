...
generateFortranCode decomposed_ast astInst idSigList =
    let        
        (   ast, 
        ...
        generatedFunctionDefs = generateDefs progDefs asts_function_defs
...
inferSignatures :: Map.Map Name FSig -> TyTraCLAST ->  [(Name,FSig)]
inferSignatures functionSignatures ast  = Map.toList (inferSignaturesMap functionSignatures ast)
...
generateNonSubDefs progDefs ast =
    let
        (   ast,
            functionSignatures,
            stencilDefinitions,
        ...
        (stmts, decl_lines,decls) = foldl (
            \(lst,st,ds) elt ->  let
                (elt',st',ds') = generateNonSubDef progDefs elt
            in
                (lst++[elt'],st++st',ds++ds')
                ) ([],[],[]) ast
...
generateDefs progDefs ast =
        fst $ foldl (
            \(lst,st) elt ->  let
                (elt',st') = generateSubDef progDefs elt st
            in
                (lst++[elt'],st')
                ) ([],[]) ast
-- generateSubDef :: Map.Map Name FSig -> (Expr, Expr) -> [String] -> (String,[String])
generateSubDef progDefs t st =
    let
        (lhs,rhs) = t
        Function ho_fname _ = lhs
    in
        (case rhs of
            MapS sv_exp f_exp -> generateSubDefMapS sv_exp f_exp ho_fname progDefs
            ApplyT f_exps -> generateSubDefApplyT f_exps ho_fname progDefs
            Comp (PElt idx) f_exp -> generateSubDefElt idx f_exp ho_fname progDefs
            Comp f1_exp f2_exp -> generateSubDefComp f1_exp f2_exp ho_fname progDefs
            FComp f1_exp f2_exp -> generateSubDefFComp f1_exp f2_exp ho_fname progDefs
            Id _ _ -> generateSubDefOpaque ho_fname progDefs
            _ -> show rhs
            ,[])
...
generateSubDefOpaque fname progDefs =
    let
        (   ast,
            functionSignatures,
            stencilDefinitions,
        ...
        opaque_function_code_strs = if null scalarisedArgs
            then
                []
            else
                let
...
generateSubDefMapS sv_exp f_exp maps_fname progDefs =
    let
        (   ast,
            functionSignatures,
            stencilDefinitions,
...
generateSubDefApplyT f_exps applyt_fname progDefs =
    let
        (   ast,
            functionSignatures,
            stencilDefinitions,
...
generateSubDefComp f1_exp f2_exp comp_fname progDefs =
    let
        (   ast,
            functionSignatures,
            stencilDefinitions,
...
generateSubDefFComp f1_exp f2_exp fcomp_fname progDefs =
    let
        (   ast,
            functionSignatures,
            stencilDefinitions,
...
generateSubDefElt idx f_exp felt_name progDefs =
    let
        (   ast,
            functionSignatures,
            stencilDefinitions,
...
generateStencilAppl s_exp v_exp@(Vec _ dt) sv_name progDefs =
    let
        (   ast,
            functionSignatures,
            stencilDefinitions,
...
generateStencilAppl s_exp v_exp@(ZipT vs_exps') sv_name progDefs = let
        (   ast,
            functionSignatures,
            stencilDefinitions,
            mainArgDecls,
...
generateVecAssign lhs rhs mainArgDecls = let
...
generateMap progDefs f_exp v_exp t = -- (Single ov_name)
    let
        (   ast,
            functionSignatures,
            stencilDefinitions,
...
generateFold :: Map.Map Name [Expr]
-> Expr -> Expr -> p -> (Expr, Expr) -> (String, [[Char]], [FDecl])
...
generateMainProgram progDefs ast_stages  =
    let
        ...
        generateStageKernel' functionSignatures  ct stage_ast  accs = let
                ...
                (uniqueGeneratedDecls, uniqueGeneratedDeclLines,generatedStmts) = generateNonSubDefs progDefs stage_ast
